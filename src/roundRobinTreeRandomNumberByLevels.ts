...
function useNextRoundRobinTreeRandomNumberForLevel(
  n: number,
  ...
  const nextPortionIndex = useNextRoundRobinTreeRandomNumberForLevel(
    portionCount,
    xValues,
    level + 1,
    random,
...
export function useNextRoundRobinTreeRandomNumber(
  ...
  return useNextRoundRobinTreeRandomNumberForLevel(n, xValues, 0, random);
}

export function main(): void {
  const n = 10;
  const xValues = [2, 3];
  const nextNumber = useNextRoundRobinTreeRandomNumber(n, xValues);
