...
describe('runRoundRobinQualitySimulation', () => {
  it('shows slightly lower first-position entropy for the round-robin variants than for the uniform baseline', () => {
    ...
    expect(result.uniform.firstValueEntropyBits).toBeGreaterThan(result.fixed.firstValueEntropyBits);
    expect(result.uniform.firstValueEntropyBits).toBeGreaterThan(result.levels.firstValueEntropyBits);
  });
  ...
  it('shows much stronger leaf clustering for the round-robin variants than for the uniform baseline', () => {
    ...
    expect(result.fixedLeafAdjacency.algorithmRate).toBeGreaterThan(result.fixedLeafAdjacency.uniformRate);
    expect(result.levelsLeafAdjacency.algorithmRate).toBeGreaterThan(result.levelsLeafAdjacency.uniformRate);
  });
  ...
  it('rejects invalid scenarios', () => {
    expect(() =>
      runRoundRobinQualitySimulation({ n: 0, fixedX: 3, xValues: [2, 3], trials: 100 }),
    ).toThrowError('n must be a positive integer');
    expect(() =>
      runRoundRobinQualitySimulation({ n: 10, fixedX: 3, xValues: [], trials: 100 }),
